...
"$LDC" -betterC -Oz -mtriple=mos -mcpu=$CPU -mattr=$MOS_MATTR -c "$HERE/bv_d.d" -of="$B/d.o"
...
cp "$(find "$HERE/rust/target" -name 'libbv_rs.a'|head -1)" "$B/librs.a"
...
grep -qE '@c_mkbig\(ptr[^)]*sret' "$B/c.ll" && echo "  -> sret pointer present (>4-byte aggregate returned by ref)" || echo "  -> (no sret marker; check IR)"
...
echo "### exit=$RC (0 = by-value struct ABI shared across all 5) ###"
exit $RC
