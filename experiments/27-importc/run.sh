#!/usr/bin/env bash
# D's ImportC (compile C directly, `-i`) on MOS. The rebuilt LDC (d5610c25) fixes the C
...
"$LDC" -betterC $LDC_PE -Oz -mtriple=mos -mcpu=$CPU -mattr=$MOS_MATTR -gcc="$SDKBIN/mos-sim-clang" -P-I"$HERE" -i --output-ll -of="$B/ldc.ll" -c "$HERE/imc.c" 2>/dev/null
...
"$LDC" -betterC $LDC_PE -Oz -mtriple=mos -mcpu=$CPU -mattr=$MOS_MATTR -gcc="$SDKBIN/mos-sim-clang" -P-I"$HERE" -i -c "$HERE/imc.c" -of="$B/imc.o"
