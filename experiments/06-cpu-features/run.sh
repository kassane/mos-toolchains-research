...
printf "  ldc   : "; "$LDC" -betterC -o- -c "$HERE/bench_d.d" --mtriple=mos -mcpu=mos6502 -vv 2>&1 | grep -oE "features '[^']*'"
printf "  ldc+  : "; "$LDC" -betterC -o- -c "$HERE/bench_d.d" --mtriple=mos -mcpu=mos6502 -mattr="$MOS_MATTR" -vv 2>&1 | grep -oE "features '[^']*'"

echo "### does -mattr change D's 6502 asm? ###"
"$LDC" -betterC -Oz -mtriple=mos -mcpu=mos6502                       -output-s -of="$B/d_plain.s"  -c "$HERE/bench_d.d"
"$LDC" -betterC -Oz -mtriple=mos -mcpu=mos6502 -mattr="$MOS_MATTR"   -output-s -of="$B/d_mattr.s"  -c "$HERE/bench_d.d"
